...
ISR(TIMER1_OVF_vect)
{
 PORTB ^= mask;  // по переполнению счётчика инвертировать бит 5 в PORTB, тем самым изменить состояние светодиода
}
 ...
 * с (counter, счётчик)  -- задаёт начальное значение счётчика.  */ 
 ...
 if (Serial.available()) {
    ...
    if (key == "d") {  // обработка команды d=значение
     ...
     TCCR1B |= StrToBin(val);      // обработка команды d=значение (установка коэф-та деления)          
     Serial.print("TCCR1B: ");
     Serial.println(val); 
    }
