...
const String modes[3] = {"0", "1", "2"}; // возможные значения режима, передаваемые в команде m=значение
...
byte mode = 0;  // Режим Normal (0) или CTC (1) или CTC с аппаратным выходом на пине 9 (2)
...
void setup() { // Инициализация
 ...
 DDRB |= 0b00000010; // пин 9 (PortB1) настроить как выход
 ...
 TCCR1B |= (1 << CS12);     // Задать коэф-т деления 256)
 ...
 * с (counter, счётчик)  -- задаёт начальное значение счётчика.  */ 
 ...
 if (Serial.available()) {
    ...
    if (val == "m") {
       Serial.println(F("m=0 or 1 sets Normal or CTC mode, m=2 sets CTC with hardware output at pin 9"));
     } 
    }
    ...
    if (key == "c") {  // обработка команды c=значение (установка нач.знач. счётчика в режиме Normal или значения в компараторе в режиме CTC)
     ...
     if (mode == 0) {  // режим Normal         
      TCCR1A = 0; //отсоединить выход компаратора от пина
     ...
     if (mode == 2) {  // режим CTC с аппаратным выходом на пине 9         
      TCCR1A |= 0b01000000; // включить для пина 9 режим изменения состояния при совпадении значения в счётчике со значением в компараторе
      TCCR1B |= (1 << WGM12); // включить режим CTC
      OCR1A = X; // значение в компараторе = принятое значение            
      // TIMSK1 |= (1 << OCIE1A);    // Разрешить прерывание по совпадению значения в счётчике со значением в компараторе
      Serial.print(F("OCR1A: "));
